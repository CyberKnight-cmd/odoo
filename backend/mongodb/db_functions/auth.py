...
async def create_user(signup_cred: SignUpRequest) -> bool:
    ...
    user = User(
        ...
        phno=signup_cred.phno,
        password=hash_keyword(signup_cred.password),
...
async def verify_login_user(
    ...
    return LoginResult(True, str(user['_id']), session_id=session_id, role=actual_role)
...
async def clear_refresh_token(
    ...
    try:
        result = await sessions.delete_one({"session_id": session_id,"user_id" : ObjectId(user_id)})
